...
class ExtractData:
    ...
    def __init__(self):
        ...
        self.history_constraint = np.full((nsubjects, nblocks, nphases, ntrials), np.nan, bool)
    ...
    def extract_data(self):
            ...
            for two, b in enumerate(self.data[(self.data.subject == s)].block.unique()):
                for three, p in enumerate(self.data[(self.data.subject == s) & (self.data.block == b) & (
                    ...
                    for four, t in enumerate(self.data[(self.data.subject == s) & (self.data.block == b) & (
                        ...
                        self.history_constraint[s, b, p, t] = self.df.pre_equalshown_secondlasttrial.values[0]
        ...
        return self.stim_left, self.stim_right, self.history_constraint, self.chosen_stim, self.outcome_value, self.confidence_value, self.correct_value, self.true_value


if __name__ == '__main__':

    data = ExtractData()
    stim_left, stim_right, history_constraint, chosen_stim, outcome_value, confidence_value, correct_value, true_value = data.extract_data()

    variables = ['stim_left', 'stim_right', 'history_constraint', 'chosen_stim', 'outcome_value', 'confidence_value', 'correct_value', 'true_value']
