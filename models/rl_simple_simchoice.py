...
class RescorlaConf(Rescorla):
    ...
    def learn_confidence_value(self, confidence):
        ...
        self.values[self.stim_chosen] += self.gamma * self.conf_PE     # self.values[self.choice_predicted] += self.gamma * self.conf_PE
...
class RescorlaConfGen(RescorlaConf):
    ...
    def learn_confidence_value(self, confidence):
        ...
        return self.values[self.stim_chosen]                            # self.values[self.choice_predicted]
...
class RescorlaPreservationConf(RescorlaConfGen):
    """model implments confidence baseline for generic (overall) confidence value"""
    ...
    def get_choice_probab(self):
        ...
        self.choice_probab = 1 / (1 + np.exp(-(self.beta + self.gamma*self.conf_values) * (self.values[self.stims[1]] - self.values[self.stims[0]])))
    ...
    def update(self, outcome, confidence):
        ...
        if np.isnan(outcome):
            self.beta += self.gamma * self.conf_values
