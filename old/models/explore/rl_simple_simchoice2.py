...
class RescorlaConf(Rescorla):
    ...
    def learn_confidence_value(self, confidence):
        ...
        conf_PE_unchosen = 1 - confidence - self.conf_values[self.stim_unchosen]
        self.conf_values[self.stim_chosen] += confidence * self.alpha_c * self.conf_PE
        self.conf_values[self.stim_unchosen] += (1 - confidence) * self.alpha_c * conf_PE_unchosen
        if self.stim_chosen == self.stims[1]:
            conf_PE1, conf_PE0 = self.conf_PE, conf_PE_unchosen
        else:
            conf_PE1, conf_PE0 = conf_PE_unchosen, self.conf_PE
        self.values[self.stims[1]] += self.choice_probab * self.gamma * conf_PE1
        self.values[self.stims[0]] += (1 - self.choice_probab) * self.gamma * conf_PE0
...
class RescorlaConfGen(RescorlaConf):
    ...
    def learn_confidence_value(self, confidence):
        ...
        conf_PE_unchosen = 1 - confidence - self.conf_values
        self.conf_values += confidence * self.alpha_c * self.conf_PE + (1 - confidence) * self.alpha_c * conf_PE_unchosen
        if self.stim_chosen == self.stims[1]:
            conf_PE1, conf_PE0 = self.conf_PE, conf_PE_unchosen
        else:
            conf_PE1, conf_PE0 = conf_PE_unchosen, self.conf_PE
