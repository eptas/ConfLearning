...
from ConfLearning.models.rl_simple_simchoice2 import RescorlaConf, RescorlaConfGen, RescorlaConfBase, RescorlaConfBaseGen, RescorlaConfZero, RescorlaConfZeroGen, RescorlaConfBaseZero, RescorlaConfBaseZeroGen
    ...
    for b in range(nblocks):
        ...
        for p in range(nphases):
            for i, t in enumerate(np.where(~np.isnan(stim_left[s, b, p]))[0]):
                ...
                model.stim_unchosen = int(stim_left[s, b, p, t]) if model.stim_chosen == int(stim_right[s, b, p, t]) else int(stim_right[s, b, p, t])
...
if __name__ == '__main__':
    ...
    for m, models in enumerate(modellist):
        ...
        for n in range(nsubjects):
            ...
            fitting.local_minima(expect[m], bounds[m], grid_range[m], grid_multiproc=True)
        ...
        pd.concat([eval("parameter_m" + str(m)), eval("fit_m" + str(m))], axis=1).to_pickle(f"../results/fittingData/fittingData_{model_names[m]}_cp_simchoice2.pkl", protocol=4)
