...
from ConfLearning.models.rl_simple_max_alpha_c import RescorlaConf, RescorlaConfGen, RescorlaConfBase, RescorlaConfBaseGen, RescorlaConfZero, RescorlaConfZeroGen, RescorlaConfBaseZero, RescorlaConfBaseZeroGen
...
bounds = [np.c_[np.array([la, lb]), np.array([ua, ub])],
          ...
          *[np.c_[np.array([la, lb, lg]), np.array([ua, ub, ug])] for _ in range(4)],
          *[np.c_[np.array([la, lb, lg, lan]), np.array([ua, ub, ug, uan])] for _ in range(4)],
]

expect = [(np.array([ua, ub]) - np.array([la, lb])) / 2,
          (np.array([ua, ub, uan]) - np.array([la, lb, lan])) / 2,
          *[(np.array([ua, ub, ug]) - np.array([la, lb, lg])) / 2 for _ in range(4)],
          *[(np.array([ua, ub, ug, uan]) - np.array([la, lb, lg, lan])) / 2 for _ in range(4)],
]

grid_range = [
    [grid_alpha, grid_beta],
    [grid_alpha, grid_beta, grid_alpha_n],
    *[[grid_alpha, grid_beta, grid_gamma] for _ in range(4)],
    *[[grid_alpha, grid_beta, grid_gamma, grid_alpha_n] for _ in range(4)]
]
...
paramlist = [
    [alpha, beta],
    [alpha, beta, alpha_n],
    *[[alpha, beta, gamma] for _ in range(4)],
    *[[alpha, beta, gamma, alpha_n] for _ in range(4)]
]
nparams = [2, 3, 3, 3, 3, 3, 4, 4, 4, 4]
    ...
    for b in range(nblocks):
        ...
        for p in range(nphases):
            for i, t in enumerate(np.where(~np.isnan(stim_left[s, b, p]))[0]):
                ...
                confidence = 1 - confidence_value[s, b, p, t] / 10
                if return_bias:
...
if __name__ == '__main__':
    ...
    for m, models in enumerate(modellist):
        ...
        pd.concat([eval("parameter_m" + str(m)), eval("fit_m" + str(m))], axis=1).to_pickle(f"../results/fittingData/fittingData_{model_names[m]}_max_alpha_c_inv.pkl", protocol=4)
