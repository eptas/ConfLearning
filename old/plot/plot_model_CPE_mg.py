...
from ConfLearning.play.test_experimental_data_simchoice_mg import run_model, RescorlaConfBaseGen
...
m = pd.read_pickle(os.path.join(path_data, f"fittingData_{winning_model}{suffix}.pkl"))
...
for i in range(5):
    handles_color[i] = plt.plot([100], [1], 's', markersize=10, mfc=colors[i], alpha=0.6, label='')[0]
...
plt.ylabel('Confidence prediction error')
...
plt.xlabel('Trial')
# plt.legend(loc='upper left', bbox_to_anchor=(1, 1), title='No. trials in Phase 1')
leg = plt.legend(handles_linestyle[::-1], ntrials_phase0[::-1], loc='upper left', title='No. trials in Phase 1', fontsize=9, title_fontsize=9.5, labelspacing=0.5, handlelength=4, frameon=False)
leg._legend_box.align = 'left'
plt.gca().add_artist(leg)
leg2 = plt.legend(handles_color[::-1], ['' for _ in range(5)], loc='upper left', bbox_to_anchor=(1.02, 1), fontsize=9, labelspacing=0.5, handletextpad=2.5, framealpha=1, title='Stimulus', title_fontsize=9.5)
...
savefig(f'../figures/learning/model_CPE_over_trials_{select_ntrials_phase1}.png', pad_inches=0.01)