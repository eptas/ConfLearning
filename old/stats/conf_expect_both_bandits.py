...
for m, models in enumerate(modellist):
    for n in range(nsubjects):
        ...
        conf_expected_value = run_model(parameter[m], models, n, return_cp=False, return_full=False, return_conf_esti=False, return_nll=False, return_bias=True)
        ...
        for k in range(nbandits):
            for b in range(nblocks):
                for p in range(nphases):
                    ...
                    if p == 0:
                        confEXP = conf_expected_value[b, p, :, k]
                        confEXP = np.hstack((np.full(np.sum(np.isnan(confEXP)), np.nan), confEXP[~np.isnan(confEXP)]))
                    else:
                        confEXP = conf_expected_value[b, p, :, k][~np.isnan(conf_expected_value[b, p, :, k])]

                    conf_exp_values = pd.DataFrame(data={"s" + str(n) + "b" + str(b) + "p" + str(p): confEXP},
                                                  columns=["s" + str(n) + "b" + str(b) + "p" + str(p)])
                    locals()["conf_exp_value" + str(k)] = pd.concat([eval("conf_exp_value" + str(k)), conf_exp_values], axis=1)
...
for k in range(nbandits):
    for p in range(nphases):
        ...
        mean_values = eval("conf_exp_value" + str(k)).filter(regex="p" + str(p)).mean(axis=1)
        locals()["mean_value" + str(k)] = np.append(eval("mean_value" + str(k)), mean_values[~np.isnan(mean_values)])

    phase_0 = eval("conf_exp_value" + str(k)).filter(regex="p" + str(0)).mean(axis=1)
    locals()["phase_zero" + str(k)] = np.append(eval("phase_zero" + str(k)), phase_0[~np.isnan(phase_0)])

    phase_1 = eval("conf_exp_value" + str(k)).filter(regex="p" + str(1))
...
plt.ylabel('expected confidence per bandit', fontweight='bold')
plt.title('expected confidence curve - ' + str(modellist[model])[38:-2])
...
plt.savefig('conf_expect_M' + str(model) + '_both_bandits.png', bbox_inches='tight')
plt.close()
