...
data = data[~data.equal_value_pair & ~data.subject.isin([25, 30])]
...
for sp in range(2):
    ...
    d2 = data[(data.b_ntrials_noc == 0) & (data.phase == 2)].groupby(['subject', 'trial_phase'])[var].mean()
    ...
    plt.plot(np.arange(nt_phase1_max-0.5, nt_phase1_max+nt_phase0phase1-10.5), m, lw=2, color=colors[0], alpha=0.6)
    d2 = data[(data.b_ntrials_noc > 0) & (data.phase == 2)].groupby(['subject', 'trial_phase'])[var].mean()
    m = d2.mean(level='trial_phase').rolling(window=window, center=True).mean().values.astype(float)
    se = d2.sem(level='trial_phase').rolling(window=window, center=True).mean().values.astype(float)
    plt.plot(np.arange(nt_phase1_max-0.5, nt_phase1_max+nt_phase0phase1-10.5), m, lw=2, color=colors[1], alpha=0.6)
