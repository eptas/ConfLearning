...
def get_data(winning_model, model_suffix, reload=False):
    ...
    if reload:
        ...
        for i, s in enumerate(include):
            ...
            CPE = np.moveaxis(run_model(params, mapping[winning_model], s, return_cp=False, return_full=False, return_conf_esti=True)[0], 2, 3)
                ...
                for p in range(3):
                    for c in range(5):
                        if c and (order[c] == order[c - 1]):
                            d.loc[(d.subject == s) & (d.block == b) & (d.phase == p), f'ABSCPE{order[c]}'] = (np.abs(CPE[b, p, c, ~np.isnan(CPE[b, p, c])]) + np.abs(CPE[b, p, c-1, ~np.isnan(CPE[b, p, c-1])])) / 2
                        else:
                            d.loc[(d.subject == s) & (d.block == b) & (d.phase == p), f'ABSCPE{order[c]}'] = np.abs(CPE[b, p, c, ~np.isnan(CPE[b, p, c])])
        d.to_pickle(os.path.join(path, f'../data/ABSCPE_{winning_model}.pkl'))
    else:
        d = pd.read_pickle(os.path.join(path, f'../data/ABSCPE_{winning_model}.pkl'))
    return d


def plot_ABSCPE(legend_phase1=True, legend_value=True, ylabel_as_title=False, markersize_value=10, title=None,
                title_xshift=0, winning_model='MonoUnspec', model_suffix='_simchoice', select_ntrials_phase1=15,
                reload=False):
    ...
    for c in range(4):
        for i, nt in enumerate(ntrials_phase0):
            d0 = d[(d.b_ntrials_pre == nt) & (d.phase == 0)].groupby(['subject', 'trial_phase_rev'])[f'ABSCPE{c}'].mean()
        ...
        for i, nt in enumerate(ntrials_phase0):
            d1 = d[(d.b_ntrials_pre == nt) & (d.phase == 1)].groupby(['subject', 'trial_phase'])[f'ABSCPE{c}'].mean()
        ...
        for i, nt in enumerate(ntrials_phase0):
            # d2 = d[(d.b_ntrials_pre == nt) & (d.phase == 2)].groupby(['subject', 'trial_phase'])[f'ABSCPE{c}'].mean()
            # m = d2.groupby(level='trial_phase').mean().values.astype(float)
            # se = d2.groupby(level='trial_phase').sem().values.astype(float)
            m = [d[(d.b_ntrials_pre == nt) & (d.phase == 2) & (d.trial_phase == t)].groupby('subject')[f'ABSCPE{c}'].mean().values.mean() for t in range(27-nt)]
    ...
    if ylabel_as_title:
        tt = plt.title('Abs. confidence prediction error')
        tt.set_position((0.5+title_xshift, 1))
    else:
        plt.ylabel('Abs. confidence prediction error')
    ...
    if legend_value:
        ...
        plt.text(1.16, 0.70, 'value level', color=(0.3, 0.3, 0.3), ha='center', rotation=90, transform=plt.gca().transAxes, fontsize=8, zorder=10)
...
if __name__ == '__main__':
    ...
    plot_ABSCPE(winning_model=winning_model, model_suffix=suffix, select_ntrials_phase1=select_ntrials_phase1,
             reload=reload)

    set_fontsize(label=12, tick=10)
    plt.tight_layout()
    plt.subplots_adjust(right=0.84)
    savefig(f'../figures/model/model_ABSCPE_over_trials_{select_ntrials_phase1}_{winning_model}.png', pad_inches=0.01)
    plt.show()