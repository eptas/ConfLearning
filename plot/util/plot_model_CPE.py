...
def plot_CPE(legend_phase1=True, legend_value=True, ylabel_as_title=False, markersize_value=10, title=None,
    ...
    for c in range(4):
        ...
        for i, nt in enumerate(ntrials_phase0):
            ...
            m = [d[(d.b_ntrials_pre == nt) & (d.phase == 2) & (d.trial_phase == t)].groupby('subject')[f'CPE{c}'].mean().values.mean() for t in range(27-nt)]
    ...
    if ylabel_as_title:
        tt = plt.title('Confidence prediction error')
...
if __name__ == '__main__':
    ...
    plot_CPE(winning_model=winning_model, model_suffix=suffix, select_ntrials_phase1=select_ntrials_phase1,
             reload=reload)

    set_fontsize(label=12, tick=10)
    plt.tight_layout()
    plt.subplots_adjust(right=0.84)
    savefig(f'../figures/model/model_CPE_over_trials_{select_ntrials_phase1}_{winning_model}.png', pad_inches=0.01)
    plt.show()