    ...
    if reload:
        ...
        for i, s in enumerate(include):
            ...
            for b in range(nblocks):
                ...
                for p in range(3):
                    d.loc[(d.subject == s) & (d.block == b) & (d.phase == p), f'BC'] = np.nanmean(BC[b, p], axis=0)[~np.isnan(np.nanmean(BC[b, p], axis=0))]
                    for c in range(5):
                        if c and (order[c] == order[c - 1]):
                            d.loc[(d.subject == s) & (d.block == b) & (d.phase == p), f'BC{order[c]}'] = (BC[b, p, c, ~np.isnan(BC[b, p, c])] + BC[b, p, c-1, ~np.isnan(BC[b, p, c-1])]) / 2
                        else:
                            d.loc[(d.subject == s) & (d.block == b) & (d.phase == p), f'BC{order[c]}'] = BC[b, p, c, ~np.isnan(BC[b, p, c])]
        d.to_pickle('data/BC.pkl')
    else:
        d = pd.read_pickle('data/BC.pkl')
