...
from ConfLearning.run_model.run_model_simchoice2 import run_model, RescorlaConfBaseGen
...
def get_data(winning_model, model_suffix, reload=False):
    ...
    if reload:
        ...
        for i, s in enumerate(include):
            ...
            BC = np.moveaxis(run_model(params, RescorlaConfBaseGen, s, return_cp=False, return_full=False, return_conf_esti=True)[2], 2, 3)
            for b in range(nblocks):
                ...
                for p in range(3):
                    ...
                    d.loc[(d.subject == s) & (d.block == b) & (d.phase == p), f'BC'] = np.nanmean(BC[b, p], axis=0)[:ntrials]
                    for c in range(5):
                        if c and (order[c] == order[c - 1]):
                            d.loc[(d.subject == s) & (d.block == b) & (d.phase == p), f'BC{order[c]}'] = np.nanmean([BC[b, p, c, :ntrials], BC[b, p, c-1, :ntrials]], axis=0)
                        else:
                            d.loc[(d.subject == s) & (d.block == b) & (d.phase == p), f'BC{order[c]}'] = BC[b, p, c, :ntrials]
        d.to_pickle(os.path.join(path, '../data/BC2.pkl'))
    else:
        d = pd.read_pickle(os.path.join(path, '../data/BC2.pkl'))
...
def plot_BC(legend_phase1=True, legend_value=True, title=None, ylabel_as_title=False, markersize_value=10, phaselabel_pos='bottom',
    ...
    if ylabel_as_title:
        ...
        plt.ylabel('Behavioral confidence')
    ...
    if legend_value and not plot_value_levels:
...
if __name__ == '__main__':
    ...
    plot_BC(select_ntrials_phase1=select_ntrials_phase1, reload=reload, plot_mean=True)

    set_fontsize(label=12, tick=10)
    plt.tight_layout()
    plt.subplots_adjust(right=0.84)
    # savefig(f'../figures/model/model_BC_over_trials_{select_ntrials_phase1}.png', pad_inches=0.01)
    plt.show()