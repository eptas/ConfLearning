...
def get_data(winning_model, model_suffix, reload=False):
    ...
    if reload:
        ...
        for i, s in enumerate(include):
            ...
            BP = np.moveaxis(run_model(params, RescorlaConfBaseGen, s, return_cp=False, return_full=False, return_conf_esti=True)[6], 2, 3)
            for b in range(nblocks):
                ...
                for p in range(3):
                    d.loc[(d.subject == s) & (d.block == b) & (d.phase == p), f'BP'] = np.nanmean(BP[b, p], axis=0)[~np.isnan(np.nanmean(BP[b, p], axis=0))]
                    for c in range(5):
                        if c and (order[c] == order[c - 1]):
                            d.loc[(d.subject == s) & (d.block == b) & (d.phase == p), f'BP{order[c]}'] = (BP[b, p, c, ~np.isnan(BP[b, p, c])] + BP[b, p, c-1, ~np.isnan(BP[b, p, c-1])]) / 2
                        else:
                            d.loc[(d.subject == s) & (d.block == b) & (d.phase == p), f'BP{order[c]}'] = BP[b, p, c, ~np.isnan(BP[b, p, c])]
        d.to_pickle('BP.pkl')
    else:
        d = pd.read_pickle('BP.pkl')

    return d


def plot_BP(legend_phase1=True, legend_value=True, ylabel_as_title=False, markersize_value=10,
    ...
    #         d0 = d[(d.b_ntrials_pre == nt) & (d.phase == 0)].groupby(['subject', 'trial_phase_rev'])[f'BP{c}'].mean()
    ...
    #     for i, nt in enumerate(ntrials_phase0):
    #         d1 = d[(d.b_ntrials_pre == nt) & (d.phase == 1)].groupby(['subject', 'trial_phase'])[f'BP{c}'].mean()
    ...
    #
    #     for i, nt in enumerate(ntrials_phase0):
    #         d2 = d[(d.b_ntrials_pre == nt) & (d.phase == 2)].groupby(['subject', 'trial_phase'])[f'BP{c}'].mean()
    ...
    # plot mean
    for i, nt in enumerate(ntrials_phase0):
        d0 = d[(d.b_ntrials_pre == nt) & (d.phase == 0)].groupby(['subject', 'trial_phase_rev'])['BP'].mean()
        m = d0.groupby(level='trial_phase_rev').mean().values.astype(float)
        plt.plot(np.arange(-nt+2, 2), m, lw=2, color='k', alpha=0.6, ls=linestyles[i])
    for i, nt in enumerate(ntrials_phase0):
        d1 = d[(d.b_ntrials_pre == nt) & (d.phase == 1)].groupby(['subject', 'trial_phase'])[f'BP'].mean()
        m = d1.groupby(level='trial_phase').mean().values.astype(float)
        plt.plot(np.arange(1, select_ntrials_phase1+1), m, lw=2, color='k', alpha=0.6, ls=linestyles[i])
    for i, nt in enumerate(ntrials_phase0):
        # d2 = d[(d.b_ntrials_pre == nt) & (d.phase == 2)].groupby(['subject', 'trial_phase'])[f'BP'].mean()
        # m = d2.groupby(level='trial_phase').mean().values.astype(float)
        m = [d[(d.b_ntrials_pre == nt) & (d.phase == 2) & (d.trial_phase == t)].groupby('subject')[f'BP'].mean().values.mean() for t in range(27-nt)]
        plt.plot(np.arange(select_ntrials_phase1, select_ntrials_phase1+nt_phase0phase1-nt), m, lw=2, color='k', alpha=0.6, ls=linestyles[i])


    if ylabel_as_title:
        plt.title('Behavioral performance')
    else:
...
if __name__ == '__main__':
    ...
    plot_BP(select_ntrials_phase1=select_ntrials_phase1, reload=reload)

    set_fontsize(label=12, tick=10)
    plt.tight_layout()
    plt.subplots_adjust(right=0.84)
    savefig(f'../figures/model/model_BP_over_trials_{select_ntrials_phase1}.png', pad_inches=0.01)
    plt.show()