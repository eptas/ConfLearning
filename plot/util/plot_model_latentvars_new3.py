...
plt.text(-0.13, 1.04, 'B', transform=ax12.transAxes, color=(0, 0, 0), fontsize=17)
...
plt.yticks(ax13.get_yticks(), [f'{v:.1f}' for v in ax13.get_yticks()])
plt.ylim(-0.3, 0.55)
plt.text(-0.13, 1.04, 'C', transform=ax13.transAxes, color=(0, 0, 0), fontsize=17)

# Plot legends
leg = plt.legend(handles_phase1, labels_phase1, loc='upper left', bbox_to_anchor=(1.03, 0.95), title='No. trials\nin Phase 1:', fontsize=9, title_fontsize=9.5, labelspacing=0.3, handlelength=4, frameon=False)
leg._legend_box.align = 'left'
ax13.add_artist(leg)
leg2 = plt.legend(handles_value, labels_value, loc='upper left', bbox_to_anchor=(1.5, 0.88), fontsize=9, labelspacing=0, handletextpad=2.5, framealpha=1, title='Stimulus:', title_fontsize=9.5)
ax13.add_artist(leg2)
plt.arrow(1.67, 0.465, 0, 0.265, color=(0.3, 0.3, 0.3), head_length=0.02, head_width=0.015, length_includes_head=True, clip_on=False, transform=plt.gca().transAxes, zorder=10, lw=0.75)
plt.text(1.74, 0.51, 'Value\nlevel', color=(0.3, 0.3, 0.3), ha='center', rotation=90, transform=ax13.transAxes, fontsize=9, zorder=10, linespacing=0.87)
...
plt.text(-0.135, 1.04, 'D', transform=ax21.transAxes, color=(0, 0, 0), fontsize=17)
...
plt.text(-0.115, 1.04, 'E', transform=ax22.transAxes, color=(0, 0, 0), fontsize=17)

ax23 = fig.add_subplot(gs[1, 10:15])
plot_MC(legend_value=False, legend_phase1=False, ylabel_as_title=True, reload=False)
plt.xticks(ax23.get_xticks(), [str(v) for v in ax23.get_xticks()])
plt.text(-0.13, 1.04, 'F', transform=ax23.transAxes, color=(0, 0, 0), fontsize=17)

ax24 = fig.add_subplot(gs[1, 15:18])
...
for tick in ax24.xaxis.get_majorticklabels():
...
#     tick.set_transform(tick.get_transform() + offset)
plt.text(-0.26, 1.04, 'G', transform=ax24.transAxes, color=(0, 0, 0), fontsize=17)
...
ax24.set_position(matplotlib.transforms.Bbox(ax24.get_position() + np.array([[0.01, 0], [0.055, 0]])))

offset = matplotlib.transforms.ScaledTranslation(2/72, 0, plt.gcf().dpi_scale_trans)
for ax in (ax11, ax12, ax13, ax21, ax22, ax23, ax24):
