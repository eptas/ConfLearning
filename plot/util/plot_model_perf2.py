        ...
        for i, s in enumerate(include):
            ...
            for b in range(nblocks):
                ...
                for p in range(3):
                    ...
                    d.loc[(d.subject == s) & (d.block == b) & (d.phase == p), f'PERF'] = np.nanmean(PERF[b, p], axis=0)[:ntrials]
                    for c in range(5):
                        if c and (order[c] == order[c - 1]):
                            d.loc[(d.subject == s) & (d.block == b) & (d.phase == p), f'PERF{order[c]}'] = np.nanmean([PERF[b, p, c, :ntrials], PERF[b, p, c-1, :ntrials]], axis=0)
                        else:
                            d.loc[(d.subject == s) & (d.block == b) & (d.phase == p), f'PERF{order[c]}'] = PERF[b, p, c, :ntrials]
        d.to_pickle(os.path.join(path, f'../data/PERF2_{winning_model}.pkl'))
    else:
        d = pd.read_pickle(os.path.join(path, f'../data/PERF2_{winning_model}.pkl'))
