...
def get_data(winning_model, model_suffix, reload=False):
    ...
    if reload:
        d = data[data.type_choice_obs][['subject', 'block', 'phase', 'trial_phase', 'trial_phase_rev', 'choice', 'b_ntrials_pre', 'b_ntrials_noc', 'equal_value_pair', 'type_choice']].copy()
        for s in range(nsubjects):
            ...
            val = np.moveaxis(run_model(params, mapping[winning_model], s, return_cp=False, return_full=True)[0], 2, 3)
...
def plot_value(legend_phase1=True, legend_value=True, ylabel_as_title=False, markersize_value=10, title=None, title_italic=False,
               winning_model='MonoUnspec', model_suffix='_simchoice', select_ntrials_phase1=15, reload=False, ylim=30):
    ...
    for c in range(4):
        ...
        for i, nt in enumerate(ntrials_phase0):
            ...
            m = [d[(d.b_ntrials_pre == nt) & (d.phase == 2) & (d.trial_phase == t)].groupby('subject')[f'value{c}'].mean().values.mean() for t in range(27-nt)]
    ...
    if ylabel_as_title:
        plt.title('Expected value', fontstyle='italic' if title_italic else None)
    else:
...
if __name__ == '__main__':
    ...
    savefig(f'../figures/model/model_value_over_trials_{select_ntrials_phase1}_{winning_model}.png', pad_inches=0.01)
    plt.show()