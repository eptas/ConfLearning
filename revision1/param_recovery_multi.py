...
import sys
sys.path.append('/home/matteo/Dropbox/python/confidence')
...
grid_alpha_n = np.arange(0.01, 0.311, 0.1)
grid_gamma = np.arange(1, 11.1, 2)    # np.arange(0.05, 0.5, 0.1)
...
def floop(pa):
    ...
    paramet = real_paras[pa]

    probab_choice = np.full((len(alpha), ndatasets, nblocks, nphases, ntrials_phase_max), np.nan)
    saveParameters, saveFitting, saveChoiceProbab = None, None, None

    parafit = np.full((len(alpha), ndatasets, len(initial_list)), np.nan)
    negll = np.full((len(alpha), ndatasets), np.nan)
    AIC, BIC = np.full((len(alpha), ndatasets), np.nan, float), np.full((len(alpha), ndatasets), np.nan)
    ...
    for l, loop in enumerate(eval(paramet.lower())):
        ...
        for i in np.arange(0, ndatasets):
            ...
            fitting.set_model(i, ndatasets, winning_model, recov_params, len(real_paras), simu_id)
            fitting.local_minima(expect, bounds, grid_range, grid_multiproc=False)

            for p in range(len(real_paras)):
                parafit[l, i, p] = min(bounds[p][1], max(bounds[p][0], fitting.data[i, p]))

            negll[l, i], probab_choice[l, i] = recov_params(fitting.data[i], winning_model, i, simu_id, return_cp=True)
            nsamples = np.sum(~np.isnan(probab_choice[l, i]))

            AIC[l, i], BIC[l, i] = fitting.model_fit(negll[l, i], nsamples)

            if i == (ndatasets - 1):

                parameter_fit = pd.DataFrame(data={"ALPHA_" + str(pa) + '_' + str(l) + 'e': parafit[l, :, 0],
                                                   "BETA_" + str(pa) + '_' + str(l) + 'e': parafit[l, :, 1],
                                                   "ALPH_N_" + str(pa) + '_' + str(l) + 'e': parafit[l, :, 2],
                                                   "GAMMA_" + str(pa) + '_' + str(l) + 'e': parafit[l, :, 3]
                                                   },
                ...
                model_fit = pd.DataFrame(data={"AIC_" + str(pa) + '_' + str(l) + 'e': AIC[l, :],
                                               "BIC_" + str(pa) + '_' + str(l) + 'e': BIC[l, :],
                                               "NEGLL_" + str(pa) + '_' + str(l) + 'e': negll[l, :]
                                               },
                ...
                choice_probability = pd.DataFrame(data={"cp_" + str(pa) + '_' + str(l) + 'e': probab_choice[l, :][~np.isnan(probab_choice[l, :])]},
                                                  columns=["cp_" + str(pa) + '_' + str(l) + 'e'])

                saveChoiceProbab = pd.concat([saveChoiceProbab, choice_probability], axis=1)

    saveParametersFitting = pd.concat([saveParameters, saveFitting], axis=1)
    return saveChoiceProbab, saveParametersFitting


if __name__ == '__main__':

    with Pool(4) as pool:
        result = pool.map(floop, range(len(real_paras)))
    # result = [None] * 4
    # for i in range(len(real_paras)):
    #     result[i] = floop(i)


    # pickle.dump(result, open('result.pkl', 'wb'))

    saveChoiceProbab = pd.concat([r[0] for r in result], axis=1)
    saveParametersFitting = pd.concat([r[1] for r in result], axis=1)
    if use_10:
        saveParametersFitting.to_pickle("fittingData_para_simu_10.pkl", protocol=4)
        saveChoiceProbab.to_pickle("choiceProbab_para_simu_10.pkl", protocol=4)
    else:
        saveParametersFitting.to_pickle("fittingData_para_simu.pkl", protocol=4)
