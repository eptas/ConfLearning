...
class GenDesign:
    ...
    def generate(self):
        ...
        for b in range(self.n_blocks):
            ...
            trial_types_pre_init = np.zeros(self.n_stim - self.ntrials_init_block_obs + 1, int)
            ...
            while nofit:
                ...
                # stim1 = np.hstack((range(self.n_stim), np.random.randint(self.n_stim)))
            ...
            seq_pairs[np.where(trial_types_pre_init == 0)[0]] = np.hstack((pairids_pre_init, np.random.choice(np.setdiff1d(range(len(self.pairs)), pairids_pre_init))))


            trial_types_prepost_remain = np.zeros(ntrials_prepost_remain)
