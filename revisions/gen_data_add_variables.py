...
pair = np.full((ndatasets, nblocks, nphases, ntrials), np.nan, float)
equal_value_pair = np.full((ndatasets, nblocks, nphases, ntrials), np.nan, float)

trial_phase = np.full((ndatasets, nblocks, nphases, ntrials), np.nan, float)
value_id = np.full((ndatasets, nblocks, nphases, ntrials), np.nan, float)
...
class ExtractData:
    ...
    def __init__(self):
        ...
        self.pairs = np.full((nblocks, nphases, ntrials), np.nan, float)
        self.equal_val_pair = np.full((nblocks, nphases, ntrials), np.nan, float)

        self.trial_p = np.full((nblocks, nphases, ntrials), np.nan, float)
        self.val_id = np.full((nblocks, nphases, ntrials), np.nan, float)
        ...
        for one, b in enumerate(self.data.block.unique()):
            for two, p in enumerate(self.data[(self.data.block == b) & (~self.data[~self.data.phase.isna()].phase.isna())].phase.unique()):
                    for three, t in enumerate(self.data[(self.data.block == b) & (self.data[~self.data.phase.isna()].phase == p) & self.data.type_choice_obs].trial_phase.astype(int).values):
                        ...
                        self.pairs[b, p, t] = self.df.pair.values[0]
                        self.equal_val_pair[b, p, t] = self.df.equal_value_pair.values[0]

                        self.trial_p[b, p, t] = self.df.trial_phase.values[0]
                        self.val_id[b, p, t] = self.df.value_id.values[0]

        return self.stim_l, self.stim_r, self.history_con, self.correct_val, self.pairs, self.equal_val_pair, self.trial_p, self.val_id
...
if __name__ == '__main__':
    ...
    for i in np.arange(0, ndatasets):
        ...
        if use_10:
            matrix = pd.read_pickle(os.path.join(path_save, 'exp_mani_' + str(i) + '_10.pkl'))
        else:
            matrix = pd.read_pickle(os.path.join(path_save, 'exp_mani_' + str(i) + '.pkl'))

        stim_left[i, :], stim_right[i, :], history_constraint[i, :], correct_value[i, :], pair[i, :], equal_value_pair[i, :], trial_phase[i, :], value_id[i, :] = data.extract_data(matrix)

    variables = ['stim_left', 'stim_right', 'history_constraint', 'correct_value', 'pair', 'equal_value_pair', 'trial_phase', 'value_id']

    for v, var in enumerate(variables):
        if use_10:
            np.save(os.path.join(path_save, f'{var}_mani_10'), eval(var))
        else:
            np.save(os.path.join(path_save, f'{var}_mani'), eval(var))
