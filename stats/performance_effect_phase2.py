...
model = regression(
    d[~d.correct.isna() & (d.phase == 1) & d.type_choice & ~d.subject.isin(exclude) & ~d.equal_value_pair],
...
# df = d[['correct', 'subject', 'block'] + list(map.values())][~d.correct.isna() & (d.phase == 1) & d.type_choice & ~d.subject.isin(exclude) & ~d.equal_value_pair]
# df.to_csv('data_phase2_correct.csv')